<div>
    <section class="admin-order my-5">
    <div class="w-full">
        ...
        <div class="flex flex-col rounded-md w-full p-8 text-gray-800 bg-white shadow-lg pin-r pin-y">
            ...
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">Invoice</th>
                    <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">Nama Pemesan</th>                    
                    <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">Jml Pembayaran</th>
                    <th class="py-4 px-6 bg-grey-lightest font-bold uppercase text-sm text-grey-dark border-b border-grey-light">Kurir</th>
            ...
            <tr>
                ...
                <td class="py-4 px-6 border-b border-grey-light">{{ $order->order_code }}</td>
                <td class="py-4 px-6 border-b border-grey-light">{{ $order->user->name }}</td>
                <td class="py-4 px-6 border-b border-grey-light">Rp. {{ number_format($order->final_price) }}</td>
                <td class="py-4 px-6 border-b border-grey-light">{{ $order->courier }}</td>
                ...
                <td class="py-4 px-6 border-b border-grey-light">
                    <a href="{{ route('detail-transaction', $order->id ) }}" class="text-grey-lighter font-bold py-1 px-3 rounded text-xl bg-green hover:bg-green-dark"><i class="fas fa-eye"></i></a>
                    @if($order->status != 0)
                        <a href="{{ route('edit-transaction', $order->id ) }}" class="text-grey-lighter font-bold py-1 px-3 rounded text-xl bg-blue hover:bg-blue-dark"><i class="fas fa-edit"></i></a>
                    @endif
                </td>
            @empty
            <tr>
